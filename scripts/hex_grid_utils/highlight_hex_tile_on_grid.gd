@tool extends HexagonalRidgeHexGrid
class_name HighlightHexTileOnGrid

@export var camera: Camera3D
@export var flat_tile_color: Color = Color("ffc60090")
@export var ridge_tile_color: Color = Color("ff004589")

var hover_hex: HexMesh = null
var highlight_overlay: MeshInstance3D

var _currently_hovered_hex: HexMesh = null
func get_currently_hovered_hex() -> HexMesh: 
	return _currently_hovered_hex

signal hex_hovered(hex: HexMesh)
...
func update_hover(new_hex: HexMesh):
	_currently_hovered_hex = new_hex
	hover_hex = new_hex
	hex_hovered.emit(new_hex)
	if hover_hex:
		var center = HexUtils.get_hex_center(hover_hex, self)
		highlight_overlay.position = center + Vector3.UP * 0.1
